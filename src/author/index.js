...
import { getAuthor, changeAuthor, getAuthorCourses, clearAuthor } from "./actions";
import alertTypes from "../shared/alert/alertTypes";
import { addAlert } from "../shared/alert/actions";

class Author extends Component {
  constructor(props) {
    super(props);
    this.state = {
      authorCoursesFetched: false
    };
  ...
  componentDidMount() {
    if (this.props.match.params.operation !== "add") {
      ...
      if (this.props.author.name && !this.state.authorCoursesFetched) {
        this.props.dispatch(getAuthorCourses(this.props.author.name));
        this.setState({ authorCoursesFetched: true });
      }
    } else {
  ...
  handleDeleteClick(event) {
    event.preventDefault();
    if (this.props.authorCourses.length !== 0) {
      this.props.dispatch(
        addAlert(
          alertTypes.WARNING,
...
function mapStateToProps(state) {
  return {
    author: state.author,
    authorCourses: state.authorCourses,
    error: state.error,
    alerts: state.alerts
  };
}
